...
function onScroll() {
  ...
  if (sc > 20) nav.classList.add("scrolled");
...
const revealSelectors =
  ".skill-card, .project-card, .edu-item, .about-img, .contact-card";
    ...
    if (window.innerWidth > 900) {
      ...
      scrollTimeout = setTimeout(() => {
        ...
        const secs = document.querySelectorAll(
          "header.full-frame, section.full-frame"
        );
        secs.forEach((s) => {
        ...
        if (nearest)
          window.scrollTo({
            top: nearest.offsetTop - navHeight,
            behavior: "smooth",
          });
      }, 160);
    }
